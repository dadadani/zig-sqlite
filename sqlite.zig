...
pub fn Iterator(comptime Type: type) type {
    return struct {
        ...
        fn readStruct(self: *Self, options: anytype) !Type {
            ...
            inline for (@typeInfo(Type).@"struct".field_names, @typeInfo(Type).@"struct".field_types, 0..) |field_name, FieldType, _i| {
                const i = @as(usize, _i);

                const ret = try self.readField(FieldType, options, i);
            ...
            ZeroBlob => {
                ...
                .@"union" => |info| {
                    ...
                    if (info.tag_type) |UnionTagType| {
                        inline for (info.field_names, info.field_types) |u_field_name, UnionFieldType| {
                            ...
                            if (field_tag == this_tag) {
                                ...
                                try self.bindField(UnionFieldType, options, u_field_name, i, field_value);
                            }
                        }
                    } else {
    ...
    fn bind(self: *Self, options: anytype, values: anytype) !void {
        ...
        switch (@typeInfo(Type)) {
            .@"struct" => |StructTypeInfo| {
                inline for (StructTypeInfo.field_names, StructTypeInfo.field_types, 0..) |struct_field_name, StructFieldType, struct_field_i| {
                    const field_value = @field(values, struct_field_name);

                    const i = sqlite3BindParameterIndex(self.stmt, struct_field_name);
                    if (i >= 0) {
                        try self.bindField(StructFieldType, options, struct_field_name, i, field_value);
                    } else {
                        try self.bindField(StructFieldType, options, struct_field_name, struct_field_i, field_value);
                    }
                }
            },
...
pub fn Statement(comptime opts: StatementOptions, comptime query: anytype) type {
    ...
    return struct {
        ...
        fn bind(self: *Self, options: anytype, values: anytype) !void {
            ...
            inline for (StructTypeInfo.field_types, 0..) |StructFieldType, _i| {
                const bind_marker = query.bind_markers[_i];
                if (bind_marker.typed) |typ| {
                    const FieldTypeInfo = @typeInfo(StructFieldType);
                    switch (FieldTypeInfo) {
                        .@"struct", .@"enum", .@"union" => comptime assertMarkerType(
                            if (@hasDecl(StructFieldType, "BaseType")) StructFieldType.BaseType else StructFieldType,
                            typ,
                        ),
                        else => comptime assertMarkerType(StructFieldType, typ),
                    }
                }
            }
